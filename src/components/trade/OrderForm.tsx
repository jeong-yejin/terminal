...
function SubmitToast({ rebate, side }: { rebate: number; side: "buy" | "sell" }) {
  return (
    <div className="animate-[reveal-up_0.25s_var(--ease-out)] flex items-center justify-center gap-2 rounded-lg bg-surface-3 px-3 py-2 text-[12px]">
      <span className="text-positive font-bold">✓ Order submitted</span>
      <span className="text-text-disabled">—</span>
      <span className="num-mono font-bold text-primary">+${rebate.toFixed(2)} rebate</span>
    </div>
  );
}
...
export function OrderForm({ symbol }: OrderFormProps) {
  ...
  return (
    <div className="flex flex-col gap-0 border-t border-border-subtle">
      ...
      {/* ── Quick settings strip / Advanced order type tabs ── */}
      {isAdvanced ? (
        ...
        /* Quick settings pill — tap to expand advanced settings */
        <div className="px-2 pt-2 pb-1.5">
          <button
            onClick={() => setIsAdvanced(true)}
            className="flex w-full cursor-pointer items-center justify-between rounded-lg bg-surface-2 px-3 py-[7px] text-[12px] transition-colors hover:bg-surface-3 focus-ring"
            title="Change order settings"
          >
            <span className="flex items-center gap-2 text-text-secondary font-medium">
              <span>{orderType}</span>
              <span className="text-border-subtle">·</span>
              <span style={{ color: levColor }} className="font-bold">{leverage}×</span>
      ...
      {/* ── Margin + Leverage row — advanced only ── */}
      {isAdvanced && (
        <div className="flex items-center border-b border-border-subtle px-2 pb-0">
          ...
          <button
            ...
            className="pb-2 pt-1.5 text-[11px] text-text-disabled hover:text-text-tertiary transition-colors cursor-pointer"
            title="Simple view"
          >
            Simple
          </button>
      ...
      <div className="flex flex-col gap-2.5 px-2 pt-2.5 pb-2">
        ...
        {/* ── Rebate estimate — updates live as user types amount ── */}
        <div
          className={`flex items-center justify-between rounded-lg border px-3 py-2 text-[12px] transition-all duration-200 ${
            rebateAmount > 0
              ? "border-primary/20 bg-primary/8 opacity-100"
              : "border-transparent bg-surface-2 opacity-50"
          }`}
        >
